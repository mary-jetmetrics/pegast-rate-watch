...
# Стоимость тура и внесённые платежи — в tour.json, а не в коде: этот файл читает
# и дашборд расходов, чтобы считать цель по туру.
...
with open(os.path.join(HERE, "tour.json"), encoding="utf-8") as _f:
    _tour = json.load(_f)
TOUR_TOTAL_EUR = _tour["total_eur"]
# дата, сумма в рублях, курс зачёта, зачтено евро
PAYMENTS = [(dt.date.fromisoformat(p["date"]), p["rub"], p["rate"], p["eur"])
            for p in _tour["payments"]]
